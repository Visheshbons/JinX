...
int main(int ArgumentC, char* ArgumentV[]) {
    if (ArgumentC < 2) {
        std::cout << "Usage: jasm input.ja [output.bin] [-flag]" << std::endl;
        return 1;
    }
    ...
    std::ifstream SomeFlag(ArgumentV[3]);
    if (SomeFlag && ArgumentV[3] == "-debug") {
        DebugFlag = true;
    } else {
        std::cerr << "Warning: unknown flag named '" << ArgumentV[3] << "'" << std::endl;
    }
